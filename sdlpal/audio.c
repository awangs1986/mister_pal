...
#if defined(__MISTER__)
enum { PAL_MISTER_AUD_CHUNK = 512 };
static volatile int g_mister_audio_run = 0;
static pthread_t g_mister_audio_thr;

static void *
PAL_Mister_AudioThread(
   void *arg
)
{
   int16_t buf[PAL_MISTER_AUD_CHUNK * 2];
   (void)arg;
   while (g_mister_audio_run && NativeVideoWriter_IsActive())
   {
      if (NativeVideoWriter_AudioSpace() < (uint32_t)PAL_MISTER_AUD_CHUNK)
      {
         usleep(2000);
         continue;
      }
      AUDIO_FillBuffer(NULL, (LPBYTE)buf, (INT)(PAL_MISTER_AUD_CHUNK * 2 * sizeof(int16_t)));
      NativeVideoWriter_WriteAudio(buf, (uint32_t)PAL_MISTER_AUD_CHUNK);
   }
   return NULL;
}
#endif
...
--*/
{
...
#if defined(__MISTER__)
   /* Feed OGG/SFX into MiSTer DDR audio ring @ 48 kHz (no SDL audio device). */
   gConfig.iSampleRate = 48000;
   gConfig.iAudioChannels = 2;
   /* Cap below max — full-scale OGG into MiSTer DAC path sounded like explode/clip. */
   gConfig.iMusicVolume = 70;
   gConfig.iSoundVolume = 70;
   /* Force OGG BGM: Games/ogg/NN.ogg (2-digit). No RIX/MIDI. */
   ...
   gAudioDevice.spec.freq = 48000;
   gAudioDevice.spec.format = AUDIO_S16SYS;
   gAudioDevice.spec.channels = 2;
   gAudioDevice.spec.samples = PAL_MISTER_AUD_CHUNK;
   gAudioDevice.pSoundBuffer = malloc(PAL_MISTER_AUD_CHUNK * 2 * sizeof(short));
   if (!gAudioDevice.pSoundBuffer)
      return -3;

   gAudioDevice.pSoundPlayer = SOUND_Init();
   gAudioDevice.pMusPlayer = OGG_Init();
   if (!gAudioDevice.pMusPlayer)
      fprintf(stderr, "MiSTer: OGG_Init FAILED — music will be silent\n");
   else
      fprintf(stderr, "MiSTer: OGG music player ready (expect Games/ogg/NN.ogg)\n");
   fflush(stderr);
   gAudioDevice.pCDPlayer = NULL;
   gAudioDevice.fOpened = TRUE;

   g_mister_audio_run = 1;
   if (pthread_create(&g_mister_audio_thr, NULL, PAL_Mister_AudioThread, NULL) != 0)
   {
      g_mister_audio_run = 0;
      UTIL_LogOutput(LOGLEVEL_WARNING, "MiSTer audio thread failed; music silent\n");
   }
   else
   {
      pthread_detach(g_mister_audio_thr);
      /* fprintf: Release builds filter LOGLEVEL_INFO via UTIL_LogOutput */
      fprintf(stderr, "MiSTer AUDIO_OpenDevice: DDR 48kHz stereo\n");
      fflush(stderr);
      UTIL_LogOutput(LOGLEVEL_INFO, "MiSTer AUDIO_OpenDevice: DDR 48kHz stereo\n");
   }
   return 0;
#endif
...
#if defined(__MISTER__)
	{
		extern void PAL_Diag_Music(int track, int loop);
		PAL_Diag_Music(iNumRIX, fLoop ? 1 : 0);
	}
#endif
	if (iNumRIX > 0)
	{
