...
#if defined(__MISTER__)
   {
      extern void PAL_Diag_Shutdown(const char* reason, int exit_code);
      PAL_Diag_Shutdown("PAL_Shutdown", exit_code);
   }
   SDL_Quit();
   UTIL_Platform_Quit();
   exit(exit_code);
#elif !__EMSCRIPTEN__ && SDL_MAJOR_VERSION < 3
      ...
      if (g_InputState.dwKeyPress & (kKeyMenu | kKeySearch))
      {
#if defined(__MISTER__)
         {
            ...
            PAL_Diag_Lifecycle(
               ...
               "splash skip via MENU(Select)" :
               "splash skip via SEARCH(Start/A)");
         }
#endif
         //
