...
SDL_ScaleMode VIDEO_GetScaleMode() {
    static SDL_ScaleMode scaleMode = -1;
    if (scaleMode == -1) {
        if (gConfig.pszScaleQuality == NULL)
            scaleMode = SDL_SCALEMODE_NEAREST;
        else
            scaleMode = SDL_atoi(gConfig.pszScaleQuality);
    }
    return scaleMode;
}
#endif
...
static void VIDEO_Mister_RenderCopy(void)
{
   extern void NativeVideoWriter_WriteFrame(const void* rgba8_pixels, int width, int height);
   if (gpScreenReal == NULL || gpScreenReal->pixels == NULL)
      return;
   if (gpScreenReal->pitch == 320 * 4)
   {
      NativeVideoWriter_WriteFrame(gpScreenReal->pixels, 320, 200);
   }
   else
   {
      static uint8_t pack[320 * 200 * 4];
      const uint8_t *src = (const uint8_t *)gpScreenReal->pixels;
      int y;
      for (y = 0; y < 200; y++)
      {
         memcpy(pack + y * 320 * 4, src + y * gpScreenReal->pitch, 320 * 4);
      }
      NativeVideoWriter_WriteFrame(pack, 320, 200);
   }
}
#endif
INT
...
--*/
{
...
#if defined(__MISTER__)
   /* Software surfaces only ¡ª flip to DDR via NativeVideoWriter (no SDL window). */
   ...
   gRenderBackend.CreateTexture = NULL;
   gRenderBackend.RenderCopy = VIDEO_Mister_RenderCopy;
   UTIL_LogOutput(LOGLEVEL_INFO,
      "MiSTer VIDEO_Startup: 320x200 software (letterbox to 320x224 in writer)\n");
   return 0;
#endif
...
#if defined(PAL_FORCE_240P)
   UTIL_LogOutput(LOGLEVEL_INFO,
      "PAL_FORCE_240P: window %ux%u textureAspect %ux%u letterbox dst=(%d,%d %dx%d)\n",
      (unsigned)gConfig.dwScreenWidth, (unsigned)gConfig.dwScreenHeight,
      (unsigned)gConfig.dwTextureWidth, (unsigned)gConfig.dwTextureHeight,
      gTextureRect.x, gTextureRect.y, gTextureRect.w, gTextureRect.h);
#endif
...
#if SDL_VERSION_ATLEAST(3,0,0) && SDL_MINOR_VERSION < 3
         SDL_SetTextureScaleMode(gpTouchOverlay, VIDEO_GetScaleMode());
#endif
#if SDL_MAJOR_VERSION<3
         SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0");
#endif
