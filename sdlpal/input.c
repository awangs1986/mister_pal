...
--*/
{
   ...
   {
      /* Defined in MiSTer_PAL/src — inject FPGA pad into g_InputState. */
      typedef struct {
         uint32_t held;
         uint32_t pressed;
         uint32_t released;
      ...
      extern void PAL_MisterInput_Poll(PAL_MisterInput* st, int player);
      static PAL_MisterInput mist;
      static BOOL mist_initialized = FALSE;
      PAL_MisterInput_Poll(&mist, 0);
      if (!mist_initialized)
      {
         /* Seed held state without turning buttons held across scene/startup
          * transitions into synthetic rising edges. */
         mist.pressed = 0;
         mist.released = 0;
      ...
      {
         g_InputState.dwKeyPress |= mist.pressed;
      }
      if (mist.dir <= 3)
      {
         g_InputState.prevdir = (mist.held &
            (kKeyUp | kKeyDown | kKeyLeft | kKeyRight))
            ? g_InputState.dir : kDirUnknown;
         g_InputState.dir = (PALDIRECTION)mist.dir;
      }
      else
      {
         g_InputState.prevdir = g_InputState.dir;
