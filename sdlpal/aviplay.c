...
)
{
...
#if defined(__MISTER__)
	{
		...
		if (PAL_MisterAuto_ShouldSkipMedia())
		{
			...
			PAL_Diag_Lifecycle(PAL_va(0, "AVI AUTOPLAY skip %s", lpszPath ? lpszPath : "?"));
			return TRUE;
		}
	}
#endif
...
#if defined(__MISTER__)
	/* Fail-safe: never block the game loop forever on a bad/long AVI. */
	const uint32_t dwAviDeadline = dwFrameStartTime + 45000; /* 45s wall clock */
	const uint32_t dwAviMaxFrames = 15 * 60;                 /* ~60s @ 15fps */
#endif

    while (!fEndPlay)
    {
#if defined(__MISTER__)
		if (SDL_GetTicks() >= dwAviDeadline || dwFrameNumber >= dwAviMaxFrames)
		{
			extern void PAL_Diag_Lifecycle(const char* event);
			PAL_Diag_Lifecycle(PAL_va(0,
				"AVI fail-safe abort %s frames=%u",
				lpszPath ? lpszPath : "?", (unsigned)dwFrameNumber));
			fEndPlay = TRUE;
			break;
		}
#endif
            ...
            if (g_InputState.dwKeyPress & (kKeyMenu | kKeySearch))
            {
#if defined(__MISTER__)
                {
                    ...
                    PAL_Diag_Lifecycle(
                        ...
                        "AVI skip via MENU" : "AVI skip via SEARCH");
                }
#endif
                fEndPlay = TRUE;
            }
