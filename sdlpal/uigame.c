...
)
{
   ...
   {
      extern int PAL_MisterAuto_ForceNewGame(void);
      if (PAL_MisterAuto_ForceNewGame())
      {
         extern void PAL_Diag_Lifecycle(const char* event);
         PAL_Diag_Lifecycle("OpeningMenu: AUTOPLAY force New Game");
         AUDIO_PlayMusic(0, FALSE, 0);
         {
            extern int PAL_MisterAuto_ShouldSkipMedia(void);
            if (!PAL_MisterAuto_ShouldSkipMedia())
               PAL_PlayAVI("3.avi");
            else
               PAL_Diag_Lifecycle("OpeningMenu: AUTOPLAY skip 3.avi");
         }
         return 0;
      }
   }
   ...
   {
      extern void PAL_Diag_Lifecycle(const char* event);
      PAL_Diag_Lifecycle("PAL_OpeningMenu enter");
   }
#endif
...
#if defined(__MISTER__)
      /* On PC, cancelling the opening menu still starts a new game. On MiSTer
       * Select=Menu, so Cancel must stay on the menu — not kick into New Game. */
      if (wItemSelected == MENUITEM_VALUE_CANCELLED)
      {
         extern void PAL_Diag_Lifecycle(const char* event);
         PAL_Diag_Lifecycle("OpeningMenu: CANCELLED ignored (stay on menu)");
         wDefaultItem = 0;
         continue;
      }
#endif
   ...
   {
      extern void PAL_Diag_Lifecycle(const char* event);
      PAL_Diag_Lifecycle("OpeningMenu: FadeOut begin");
   }
#endif
   PAL_FadeOut(1);
#if defined(__MISTER__)
   {
      extern void PAL_Diag_Lifecycle(const char* event);
      PAL_Diag_Lifecycle("OpeningMenu: FadeOut done");
   }
#endif
   ...
   {
#if defined(__MISTER__)
      /* 3.avi (MSVC 288x180) has hung / killed the ARM frontend after New Game.
       * Skip on MiSTer so manual play proceeds into GameMain; PC keeps AVI. */
      {
         extern void PAL_Diag_Lifecycle(const char* event);
         PAL_Diag_Lifecycle("OpeningMenu: New Game → skip 3.avi (MiSTer fail-safe)");
      }
#else
      PAL_PlayAVI("3.avi");
#endif
      ...
      {
         extern void PAL_Diag_Lifecycle(const char* event);
         PAL_Diag_Lifecycle("OpeningMenu: New Game → return to GameMain");
      }
#endif
   }
#if defined(__MISTER__)
   ...
   {
      extern void PAL_Diag_Lifecycle(const char* event);
      PAL_Diag_Lifecycle(PAL_va(0, "OpeningMenu: Load slot %d", (int)wItemSelected));
   }
#endif
...
)
{
	...
	if (wReturnValue == 1 || wReturnValue == 2)
	{
		...
		{
			extern void PAL_Diag_Lifecycle(const char* event);
			PAL_Diag_Lifecycle(wReturnValue == 2
				? "PAL_QuitGame confirmed → launch settings then Shutdown"
				: "PAL_QuitGame confirmed → Shutdown/exit (handler relaunch = title bounce)");
		}
#endif
